...
async def clinical_operations_query(
    ...
    try:
        ...
        if patient_context:
            messages.append({"role": "user", "content": f"Patient Context: {patient_context}"})
        ...
        return {
            "success": False,
            "error": str(e),
            "api_key_provided": OPENAI_API_KEY[:10] + "..." if OPENAI_API_KEY else "NO KEY",
...
class HealthcareAgentIntegration:
    ...
    def __init__(self):
        ...
        self.client = client
        logger.info(f"HealthcareAgentIntegration initialized with API key: {self.api_key[:10] if self.api_key else 'NO KEY'}...")
